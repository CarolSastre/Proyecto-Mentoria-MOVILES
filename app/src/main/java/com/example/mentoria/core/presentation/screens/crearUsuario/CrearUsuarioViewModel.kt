...
class CrearUsuarioViewModel(
    ...
    private val _events = Channel<CrearUsuarioUiEvent>()
    val events = _events.receiveAsFlow()

    fun onAction(action: CrearUsuarioUiAction) {
        when (action) {
            is CrearUsuarioUiAction.OnRegisterClick -> register(
                action.usuarioDto,
    ...
    private fun register(usuario: UsuarioDto, passwordConfirmation: String) {
        ...
        viewModelScope.launch {
            ...
            result.onSuccess {
                ...
                _events.send(CrearUsuarioUiEvent.RegisterSuccess)
            }.onFailure { exception ->
                _uiState.update {
                    it.copy(
                        isLoading = false,
